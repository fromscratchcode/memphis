...
impl BuiltinTypes {
    ...
    pub fn init(heap: &mut Heap) -> BuiltinTypes {
        ...
        builtin_types.write_name_to_type_classes(heap);
        builtin_types
    }

    fn write_name_to_type_classes(&self, heap: &mut Heap) {
        for type_ in Type::all().iter() {
            let name_ref =
                heap.allocate(HeapObject::new(self.str, VmValue::Str(type_.to_string())));

            let type_ref = self.get(type_);
