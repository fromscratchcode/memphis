...
pub fn eval(text: &str) -> MemphisValue {
    ...
    let value = context.run_inner().expect("Failed to evaluate test string");
...
pub fn read(context: &VmContext, name: &str) -> MemphisValue {
    let value = context.read_inner(name).expect("Failed to read variable.");
    context.vm().normalize_vm_value(value)
}
