...
"#;
    ...
    #[test]
    fn regular_import() {
        let ctx = run_path("memphis/src/fixtures/imports/regular_import.py");
        ...
        let ctx = run_path("memphis/src/fixtures/imports/regular_import_b.py");
    ...
    #[test]
    fn selective_import() {
        let ctx = run_path("memphis/src/fixtures/imports/selective_import_a.py");
        assert_read_eq!(ctx, "x", int!(5));

        let ctx = run_path("memphis/src/fixtures/imports/selective_import_b.py");
        assert_read_eq!(ctx, "y", int!(6));
        assert_read_eq!(ctx, "z", int!(6));

        let e = run_path_expect_error("memphis/src/fixtures/imports/selective_import_c.py");
        assert_name_error!(e.exception, "something_third");

        let ctx = run_path("memphis/src/fixtures/imports/selective_import_d.py");
        assert_read_eq!(ctx, "z", int!(8));

        let ctx = run_path("memphis/src/fixtures/imports/selective_import_e.py");
        assert_read_eq!(ctx, "x", int!(8));

        let ctx = run_path("memphis/src/fixtures/imports/selective_import_f.py");
        assert_read_eq!(ctx, "y", int!(6));
        assert_read_eq!(ctx, "z", int!(6));
    }
    ...
    fn selective_import_relative() {
        let ctx = run_path("memphis/src/fixtures/imports/relative/main_a.py");
        assert_read_eq!(ctx, "x", int!(2));
    }

    #[test]
    fn regular_import_relative_parent_package() {
        let ctx = run_path("memphis/src/fixtures/imports/relative/main_b.py");
        assert_read_eq!(ctx, "x", int!(2));
    }

    #[test]
    fn regular_import_relative_alias() {
        let ctx = run_path("memphis/src/fixtures/imports/relative/main_c.py");
        assert_read_eq!(ctx, "x", int!(2));
    }

    #[test]
    fn relative_import_from_package() {
        let ctx = run_path("memphis/src/fixtures/imports/pkg_test/test_app.py");
...
"#;
        ...
        assert_read_eq!(ctx, "a", float!(3.84));
        assert_read_eq!(ctx, "b", float!(3.8425));
...
"#;
    ...
    #[test]
    fn call_stack() {
        let e = run_path_expect_error("memphis/src/fixtures/call_stack/call_stack.py");
        ...
        let e = run_path_expect_error("memphis/src/fixtures/call_stack/call_stack_one_file.py");
        assert_name_error!(e.exception, "unknown");
...
"#;
    ...
    fn name_dunder_script() {
        let ctx = run_path("memphis/src/fixtures/name_dunder/main.py");
    ...
    fn package_dunder_namespace() {
        ...
        let ctx = run_path("memphis/src/fixtures/package_dunder/namespace/main.py");
    ...
    fn package_dunder_regular() {
        ...
        let ctx = run_path("memphis/src/fixtures/package_dunder/regular/main.py");
        assert_read_eq!(ctx, "__package__", none!());
        assert_read_eq!(ctx, "second_pkg", none!());
        assert_read_eq!(ctx, "third_pkg", str!("inner"));
...
"#;
    ...
    fn stdout_tests() {
        let output = run_script("fixtures/test.py");
        assert_eq!(output, include_str!("../../../fixtures/test.stdout"));

        let output = run_script("fixtures/async/a.py");
        assert_eq!(output, include_str!("../../../fixtures/async/a.stdout"));

        let output = run_script("fixtures/async/b.py");
        assert_eq!(output, include_str!("../../../fixtures/async/b.stdout"));

        let output = run_script("fixtures/async/c.py");
        assert_eq!(output, include_str!("../../../fixtures/async/c.stdout"));

        let output = run_script("fixtures/async/d.py");
        assert_eq!(output, include_str!("../../../fixtures/async/d.stdout"));

        let output = run_script("fixtures/async/e.py");
        assert_eq!(output, include_str!("../../../fixtures/async/e.stdout"));

        let output = run_script("fixtures/async/simple.py");
        assert_eq!(
            output,
            include_str!("../../../fixtures/async/simple.stdout")
        );

        let output = run_script("fixtures/exceptions.py");
        assert_eq!(output, include_str!("../../../fixtures/exceptions.stdout"));

        let output = run_script("fixtures/context_manager.py");
        assert_eq!(
            output,
            include_str!("../../../fixtures/context_manager.stdout")
        );
